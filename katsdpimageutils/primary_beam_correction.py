...
class Circular_Beam(JimBeam):
    ...
    def get_coordinates(self, phase_center, image_wcs):
        ...
        row, col = np.indices((image_wcs.array_shape[2], image_wcs.array_shape[3]))
        p2w = image_wcs.pixel_to_world(col, row, 0, 0)[0]
        separation_deg = p2w.separation(phase_center).deg
        x, y = np.meshgrid(separation_deg[0], separation_deg[1])
        return x, y

    def central_freq(self, path):
    ...
    def cosine_power_pattern(self, x, y, c_freq):
    ...
    def beam_pattern(self, path):
        ...
        c_freq = self.central_freq(path)
        # Get radial separation between sources and the phase centre
        phase_center, image_wcs = self.get_position(path)
        x, y = self.get_coordinates(phase_center, image_wcs)
        beam_list = self.cosine_power_pattern(x, y, c_freq)
        return beam_list
    ...
    def inverse_variance(self, data):
        ...
        if len(data) == 0:
            return 0.0
        med, sd = self.standard_deviation(data)
        for i in range(50):
            old_sd = sd
    ...
    def weighted_average(self, arr, weights):
        """Compute weighted average of all the frequency planes."""
        wt_average = np.average(arr, weights=weights, axis=0)
        return wt_average

    def primary_beam_correction(self, beam_pattern, raw_image, px_cut=0.1):
        ...
        for i in range(len(beam_pattern)):
            ...
            weight.append(self.inverse_variance(np.ravel(raw_image.data[0, i+nterm, :, :])))
        ...
        corr_image = self.weighted_average(pbc_image, weight)
        # Add new axis
        corr_image = corr_image.reshape(1, 1, raw_image.data.shape[2], raw_image.data.shape[3])
        return corr_image

    def _get_value_from_history(self, keyword, header):
        """
    ...
    def write_new_fits(self, pbc_image, path, outputFilename):
        """
        ...
        try:
            if 'CLEANBMJ' in newhdr and newhdr['CLEANBMJ'] > 0:
                ...
                newhdr['BMAJ'] = float(self._get_value_from_history('BMAJ', newhdr))
                newhdr['BMIN'] = float(self._get_value_from_history('BMIN', newhdr))
                newhdr['BPA'] = float(self._get_value_from_history('BPA', newhdr))
        except KeyError:
